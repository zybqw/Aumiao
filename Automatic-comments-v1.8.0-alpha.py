...
def login():
    ...
    if response.status_code == 200:
        ...
        if has_config_file() or path.exists(path.join(file_path, qwq.txt)):
            remove(CONFIG_in)
            print("不能登录或地址不存在,config.ini已删除！请重试")
        return None
...
def main():
    ...
    if reply_sign == 0 and "R" in lcr:
        ...
        if reply_sign == "Y":
            infos = 0
            lf_num = get(
                "https://api.codemao.cn/web/message-record?query_type=LIKE_FORK&limit=10&offset=999999999",
                headers=headers,
            )
            cr_num = get(
                "https://api.codemao.cn/web/message-record?query_type=COMMENT_REPLY&limit=10&offset=999999999",
                headers=headers,
            )
            while True:
                infos += 200
                ...
                if (
                    loads(lf_num.text)["total"] <= infos
                    or loads(lf_num.text)["total"] == 0
                    and loads(cr_num.text)["total"] <= infos
                    or loads(cr_num.text)["total"] == 0
                ):
                    break
            print("清除成功")
    ...
    while True:
        ...
        if "R" in lcr:
            ...
            record = get(
                "https://api.codemao.cn/web/message-record/count", headers=headers
            )
            if loads(record.text)[0]["count"] >= 1:
                print("侦测到" + str(loads(record.text)[0]["count"]) + "个新回复")
                ...
                for infos in range(loads(record.text)[0]["count"]):
                    first_reply = loads(loads(new.text)["items"][infos]["content"])[
                        "message"
                    ]
                        ...
                        if response.status_code == 201:
                            print(
                                "\n".join(
                                    [
                                        "回复成功",
                                        "回复作品  {}".format(first_reply["business_id"]),
                                        "回复对象  {} {}".format(
                                            loads(loads(new.text)["items"][infos]["content"])["sender"]["nickname"],
                                            loads(new.text)["items"][infos]["sender_id"]
                                        ),
                                        "回复内容  {}\n".format(first_reply["reply"]),
                                        f"评论内容  {answer}",
                                    ]
                                )
                            )

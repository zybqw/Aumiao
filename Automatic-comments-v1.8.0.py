...
def login():
    ...
    response = ses.post(
        ...
        headers=HEADERS,
        data=dumps({"identity": USERDICT["username"], "password": USERDICT["password"], "pid": pid}),
    )
...
def reply_work():
    ...
    if loads(record.text)[0]["count"] >= 1:
        ...
        for infos in range(loads(record.text)[0]["count"]):
            ...
            if loads(new.text)["items"][infos]["type"] == "WORK_REPLY_REPLY_FEEDBACK" or loads(new.text)["items"][infos]["type"] == "WORK_REPLY_REPLY":
